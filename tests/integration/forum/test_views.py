...
def test_user_must_be_logged_in(application, forum, topic):
    application.config['WTF_CSRF_ENABLED'] = False

    with application.test_client() as test_client:
        manage_forum_url = url_for("forum.manage_forum", forum_id=forum.id)

        login_response = test_client.post(manage_forum_url,
                        follow_redirects=True)
        ...
        assert response.status_code == 200
        assert "You are not allowed to" in response.get_data(as_text=True)

def test_user_needs_moderator_permissions(application, forum, topic, user):
    application.config['WTF_CSRF_ENABLED'] = False

    with application.test_client() as test_client:
        manage_forum_url = url_for("forum.manage_forum", forum_id=forum.id)

        login_response = test_client.post(url_for('auth.login'), data={'login': user.username, 'password': 'test'},
                        follow_redirects=True)
...
def test_highlight_topic(application, forum, topic, moderator_user):
    application.config['WTF_CSRF_ENABLED'] = False

    with application.test_client() as test_client:
        manage_forum_url = url_for("forum.manage_forum", forum_id=forum.id)

        login_response = test_client.post(url_for('auth.login'), data={'login': moderator_user.username, 'password': 'test'},
                        follow_redirects=True)
